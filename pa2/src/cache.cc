        ...
        void CACHE::handle_prefetch()
        {
            ...
            for (uint32_t i=0; i<MAX_READ; i++) {
                ...
                if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0))
                {
                    ...
                    // access cache
                    uint32_t set = get_set(PQ.entry[index].address,PQ.entry[index].cpu);
       ...
       /* uint32_t CACHE::get_set(uint64_t address, uint32_t cpu)
...
#endif
                uint32_t set_index = (uint32_t)(address & ((1 << lg2(NUM_SET)) - 1));
                uint32_t upper_1_bit = (address >> lg2(NUM_SET)) & 0x1;
                uint32_t final_set;
                if (upper_1_bit && cpu) {
                    final_set = set_index % no_sets[upper_1_bit]*NUM_SET; 
}                else {
                final_set = (set_index % no_sets[upper_1_bit]*NUM_SET) + no_sets[upper_1_bit]*NUM_SET; 
}
        ...
        /*uint32_t CACHE::get_set(uint64_t address, uint32_t cpu)
        {
            ...
            if(cache_type == IS_DTLB_PB)
                ...
                uint32_t upper_1_bit = (address >> lg2(NUM_SET)) & 0x1;
                uint32_t final_set;
                if (upper_1_bit == 1 && cpu) {
                    final_set = set_index % NUM_SET/2; 
}                else {
                final_set = (set_index % NUM_SET/2) + NUM_SET/2; 
}
        ...
        {
            uint32_t set = get_set(inval_addr,0);
            int match_way = -1;
