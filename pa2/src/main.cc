...
uint32_t llc_core_access[NUM_CPUS] = {0};
uint32_t llc_core_miss[NUM_CPUS] = {0};
uint32_t no_sets[NUM_CPUS] = {1/NUM_CPUS};
...
//	}
    ...
    cout<<"\n Initialise core being called ..... \n";
	ooo_cpu[i].initialize_core();

        // BRANCH PREDICTOR
        cout<<"\n Initialise branch predictor being called ..... \n";
        ooo_cpu[i].initialize_branch_predictor();

		ooo_cpu[i].BTB.cpu = i;
		ooo_cpu[i].BTB.cache_type = IS_BTB;

        cout<<"\n Initialise btb replacement being called ..... \n";
...
cout<<"\n Initialise STLB being called ..... \n";
    ...
    cout<<" \n Loop ran twice ... and simulation yet to being ... ";
	...
	if(cs_index >= index)
		...
		for (uint32_t j=0; j<NUM_TYPES; j++) {
			llc_total_miss += uncore.LLC.sim_miss[i][j];
		}
	...
	/*	
		...
		ooo_cpu[i].roi_store_gen = ooo_cpu[i].sim_store_gen;
                ...
                //uint32_t llc_core_access[NUM_CPUS], llc_core_miss[NUM_CPUS];

                for(uint32_t j = 0; j<NUM_CPUS ; j++){
                for (uint32_t i = 0; i < NUM_TYPES; i++) {
                    llc_core_access[j] += uncore.LLC.sim_access[j][i];
                    llc_core_miss[j] += uncore.LLC.sim_miss[j][i];
                }}

                for (uint32_t j = 0; j<NUM_CPUS ; j++) {
                    if (( int miss_ratio = llc_core_miss[j]/llc_core_access[j]) > 0.5 && llc_core_access[j] > 500){
                        no_sets[j] += 0.05  + miss_ratio;
                        for (uint32_t i = 0; i < NUM_CPUS ; i++){
                                if(i==j)
                                    continue;
                                no_sets[i] -= 0.05/(NUM_CPUS-1);
                        }
