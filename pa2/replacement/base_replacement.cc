...
uint32_t CACHE::lru_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type)
{
    ...
    uint32_t msb_pos = 31 - __builtin_clz(set);
...
/*uint32_t CACHE::lru_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type)
{
    ...
    // Define partition range based on CPU
    uint32_t way_start = (cpu == 0) ? 0 : NUM_WAY / 2;
    uint32_t way_end = (cpu == 0) ? (NUM_WAY / 2) : NUM_WAY;

    // Fill invalid line first within partition
    for (way = way_start; way < way_end; way++) {
        if (block[set][way].valid == false) {
            ...
            break; // found invalid, exit loop
        }
    }

    // If no invalid, find LRU victim within partition
    if (way == way_end) {
        uint32_t victim_way = way_start;
        uint32_t max_lru = block[set][way_start].lru;

        for (way = way_start + 1; way < way_end; way++) {
    ...
    // If still not found (should not happen), assert
    if (way == way_end) {
        cerr << "[" << NAME << "] " << __func__ << " no victim! set: " << set << " for CPU: " << cpu << endl;
        assert(0);
    }
